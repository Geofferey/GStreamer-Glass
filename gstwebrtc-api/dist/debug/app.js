(() => {
  const FRONTEND_VERSION = '3.7.39-jitter-units-fix';
  console.info(`[GStreamer Glass Debug] frontend ${FRONTEND_VERSION}`);
  const $ = (id) => document.getElementById(id);
  const wsUrl = $('wsUrl');
  const stunUrl = $('stunUrl');
  const connectBtn = $('connectBtn');
  const keepAliveSec = $('keepAliveSec');
  const jitterMs = $('jitterMs');
  ...
  function defaultKeepAliveSeconds() {
    const value = getQueryParam('keepalive') || getQueryParam('ka') || String(configValue('keepAliveSeconds', 15));
    const parsed = Number.parseInt(value, 10);
    return Number.isFinite(parsed) && parsed >= 0 ? Math.min(parsed, 300) : 15;
  }

  function defaultJitterMs() {
    const value = getQueryParam('jitter') || getQueryParam('jitterMs') || getQueryParam('jbuf') || String(configValue('playerJitterMs', 80));
    const parsed = Number.parseInt(value, 10);
    return Number.isFinite(parsed) && parsed >= 0 ? Math.min(parsed, 3000) : 80;
  }

  function getPlayerJitterMs() {
    const parsed = Number.parseInt(jitterMs && jitterMs.value ? jitterMs.value : '0', 10);
    return Number.isFinite(parsed) && parsed > 0 ? Math.max(0, Math.min(parsed, 3000)) : 0;
  }

  function applyReceiverJitter(receiver) {
    const ms = getPlayerJitterMs();
    if (!receiver || ms <= 0) return;
    const legacySeconds = ms / 1000;
    try {
      if ('jitterBufferTarget' in receiver) {
        // jitterBufferTarget is expressed in milliseconds.
        receiver.jitterBufferTarget = ms;
        log(`receiver jitterBufferTarget set to ${ms}ms`);
      } else if ('playoutDelayHint' in receiver) {
        // playoutDelayHint is the legacy seconds-based fallback.
        receiver.playoutDelayHint = legacySeconds;
        log(`receiver playoutDelayHint set to ${legacySeconds}s (${ms}ms)`);
      } else {
        log(`browser does not expose receiver jitter target; requested ${ms}ms`, 'warn');
      }
    } catch (err) {
      log(`receiver jitter target failed: ${err.message}`, 'warn');
    }
  }
  ...
  async function startConsumer(peerId) {
    ...
    pc.addEventListener('track', (ev) => {
      applyReceiverJitter(ev.receiver);
  ...
  function writeStats(lines) {
    if (statsBox) statsBox.textContent = lines.filter(Boolean).join('\n');
  }
    ...
    state.statsTimer = setInterval(async () => {
      ...
      try {
        ...
        stats.forEach((report) => {
          ...
          if (report.type === 'inbound-rtp' && (report.kind === 'video' || report.mediaType === 'video')) inboundVideo = report;
        });
        let pairLine = 'selected ICE pair: —';
        let rttLine = 'RTT —';
        if (selected) {
          ...
          pairLine = `selected ICE ${proto.toUpperCase()} local=${local ? (local.address || local.ip || '?') + ':' + (local.port || '?') : '?'} remote=${remote ? (remote.address || remote.ip || '?') + ':' + (remote.port || '?') : '?'}`;
          if (Number.isFinite(selected.currentRoundTripTime)) rttLine = `RTT ${fmtMs(selected.currentRoundTripTime)}`;
          log(pairLine);
        }
        let fpsLine = 'FPS —';
        let videoLine = 'video stats —';
        let jbufLine = 'jitter buffer —';
        if (inboundVideo) {
          ...
          if (state.lastStatsVideo && frames >= state.lastStatsVideo.frames) {
            ...
            if (Number.isFinite(fps)) fpsLine = `FPS ${fps.toFixed(1)}`;
          }
          ...
          videoLine = `jitter ${fmtMs(jitterMs, true)} loss ${lost}/${received} decoded ${frames} dropped ${dropped} freezes ${freezes}`;
          if (Number.isFinite(inboundVideo.jitterBufferDelay) && Number.isFinite(inboundVideo.jitterBufferEmittedCount) && inboundVideo.jitterBufferEmittedCount > 0) {
            jbufLine = `jitterBuffer avg ${fmtMs(inboundVideo.jitterBufferDelay / inboundVideo.jitterBufferEmittedCount)} target/hint ${getPlayerJitterMs()}ms`;
          }
        }
        writeStats([pairLine, rttLine, fpsLine, videoLine, jbufLine]);
      } catch (_) {}
    }, 1000);
  }
  ...
  if (jitterMs) jitterMs.value = String(defaultJitterMs());
  ...
  renderProducers();
  updateSessionUi();
  log('debug viewer v3.7.31 loaded; signalling defaults to TCP/WebSocket 8189; media should negotiate separately over UDP through ICE');
  log(`default signalling URL: ${wsUrl.value}`);
  log(`default keepalive interval: ${keepAliveSec ? keepAliveSec.value : '15'}s`);
  log(`default player jitter target: ${jitterMs ? jitterMs.value : '0'}ms`);
