...
if ($index -notmatch [regex]::Escape('player.css?v=3.8.29') -or
    $serviceWorker -notmatch [regex]::Escape('player.css?v=3.8.29')) {
...
if ($serviceWorker -notmatch "CACHE_NAME\s*=\s*'gstglass-pwa-3\.8-viewer-auth-56'") {
