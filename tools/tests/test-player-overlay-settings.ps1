...
foreach ($id in @('viewerSettingsPanel', 'viewerSettingStatus', 'viewerSettingStats', 'viewerSettingControls', 'viewerSettingDebug')) {
...
foreach ($setting in @('statusOverlay', 'statsOverlay', 'playbackControls', 'debugShortcut')) {
...
Assert-OverlaySettings ($serviceWorker.Contains("gstglass-pwa-3.8-viewer-auth-50")) 'Service-worker cache was not advanced for viewer settings.'
