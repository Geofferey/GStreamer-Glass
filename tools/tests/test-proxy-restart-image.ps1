...
function Invoke-PausedProxyResponse(
    ...
    try {
        ...
        return @{ Page = $page; Mp4 = $mp4; Webm = $webm; PausedStatus = $pausedStatus; ResumedStatus = $resumedStatus }
    }
    finally { try { $proxy.Stop() } catch {} }
}

$responses = Invoke-PausedProxyResponse $restartImagePath $restartMp4Path $restartWebmPath
...
$customMountResponse = (Invoke-PausedProxyResponse $restartImagePath $restartMp4Path $restartWebmPath '/watch').Page
