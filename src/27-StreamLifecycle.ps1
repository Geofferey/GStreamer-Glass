...
function Start-GstStream {
    ...
    $queueLeakControls = @(
        ...
        [pscustomobject]@{ Label = 'audio input'; Combo = $cmbAudioInputQueueLeakMode; Effective = (Get-EffectiveAudioInputQueueLeakValue) }
        [pscustomobject]@{ Label = 'audio output'; Combo = $cmbAudioOutputQueueLeakMode; Effective = (Get-EffectiveAudioOutputQueueLeakValue) }
    )
    $queueLeakSummary = ($queueLeakControls | ForEach-Object {
        "$($_.Label) $([string]$_.Combo.SelectedItem) (effective '$($_.Effective)')"
    }) -join ', '
    Append-Log "Threading: queue leak -- $queueLeakSummary."
    foreach ($queueLeakControl in $queueLeakControls) {
        if ([string]$queueLeakControl.Combo.SelectedItem -eq 'No leak - block' -and $queueLeakControl.Effective -ne 'no') {
            Append-Log "Threading guard: $($queueLeakControl.Label) queue No leak/block was selected but coerced to downstream/drop-old outside Blocking diagnostic profile."
        }
    }
